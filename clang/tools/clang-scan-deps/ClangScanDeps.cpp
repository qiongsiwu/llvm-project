    ...
    while (auto MaybeInputIndex = GetNextInputIndex()) {
      ...
      if (Format == ScanningOutputFormat::Make) {
        ...
        if (Names.size() == 1) {
          ...
          assert(Names.size() > 1 && "No name to scan!");
          unsigned NameIdx = 0;
          ...
          auto deliverResult =
              [&](StringRef Name,
                  std::optional<dependencies::TranslationUnitDeps> Result) {
                llvm::Expected<TranslationUnitDeps> MaybeTUDeps =
                    Result ? llvm::Expected<TranslationUnitDeps>(
                                 std::move(*Result))
          ...
          if (!WorkerTool.computeDependenciesWithDrain(
