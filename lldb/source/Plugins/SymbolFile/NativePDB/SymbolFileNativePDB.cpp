...
lldb::FunctionSP SymbolFileNativePDB::CreateFunction(PdbCompilandSymId func_id,
  ...
  if (ts) {
    if (PdbAstBuilder *ast_builder = ts->GetNativePDBParser())
      ast_builder->GetOrCreateFunctionDecl(func_id);
  }
  return func_sp;
}
...
VariableSP SymbolFileNativePDB::CreateGlobalVariable(PdbGlobalSymId var_id) {
  ...
  if (ts) {
    if (PdbAstBuilder *ast_builder = ts->GetNativePDBParser())
      ast_builder->GetOrCreateVariableDecl(var_id);
  }
...
VariableSP SymbolFileNativePDB::CreateLocalVariable(PdbCompilandSymId scope_id,
  ...
  if (!is_param) {
    ...
    if (ts) {
      if (PdbAstBuilder *ast_builder = ts->GetNativePDBParser())
        ast_builder->GetOrCreateVariableDecl(scope_id, var_id);
    }
  }
...
TypeSP SymbolFileNativePDB::CreateTypedef(PdbGlobalSymId id) {
  ...
  if (!ast_builder)
    return nullptr;
  ast_builder->GetOrCreateTypedefDecl(id);
...
CompilerDecl SymbolFileNativePDB::GetDeclForUID(lldb::user_id_t uid) {
  ...
  if (!ast_builder)
    return CompilerDecl();
  if (auto decl = ast_builder->GetOrCreateDeclForUid(uid))
    return *decl;
  return CompilerDecl();
}
...
bool SymbolFileNativePDB::CompleteType(CompilerType &compiler_type) {
  ...
  if (ast_builder->GetClangASTImporter().CanImport(compiler_type))
